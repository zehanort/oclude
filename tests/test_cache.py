...
def test_kernel_files_same_name():
    ...
    # run first kernel
    output1, error1, retcode1 = run_command(f"oclude -f {kernel1} -g {GSIZE} -l {LSIZE} -k vadd")

    # run second kernel
    output2, error2, retcode2 = run_command(f"oclude -f {kernel2} -g {GSIZE} -l {LSIZE} -k vmul")
...
def test_no_cache_warnings():
    ...
    _, error2, retcode2 = run_command(f"oclude -f {kernel1} -g {GSIZE} -l {LSIZE} -k vadd --no-cache-warnings")
