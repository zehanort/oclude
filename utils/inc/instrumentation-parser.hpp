...
inline instrumentation_t get_instrumentation_info_from_module() {
    ...
    for (llvm::Module::const_iterator func = m->begin(); func != m->end(); func++) {
        ...
        print_message("reporting about function " + funcname + (is_kernel ? " (kernel function) " : " ") + "at line " + std::to_string(funcline));
            ...
            for (llvm::BasicBlock::const_iterator instr = bb->begin(); instr != bb->end(); instr++) {
                ...
                if (loc.getLine() != 0) {
                    ...
                    if (llvm::isa<llvm::LoadInst>(instr))
                    	bb_instrumentation.push_back(line + ':' + resolve_memop("load", instr, is_kernel));
                    else if (llvm::isa<llvm::StoreInst>(instr))
                    	bb_instrumentation.push_back(line + ':' + resolve_memop("store", instr, is_kernel));
                    else
                        bb_instrumentation.push_back(std::to_string(loc.getLine()) + ':' + instr->getOpcodeName());
                }
            }

            print_message("\tDONE reporting about Basic Block #" + std::to_string(i-1) + " which started at line " + std::to_string(bbline));
